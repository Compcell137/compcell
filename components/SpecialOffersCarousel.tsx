...
export default function SpecialOffersCarousel({ sales, loading = false, isAdmin = false }: SpecialOffersCarouselProps) {
      ...
      <div className="w-full space-y-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4 md:gap-5 lg:gap-6">
          {currentSales.map((sale) => (
            <div key={sale.id} className="bg-black/60 backdrop-blur-sm rounded-lg border border-orange-400/40 p-2 sm:p-2.5 md:p-3 hover:border-orange-300 transition-all duration-300 hover:shadow-lg hover:shadow-orange-500/20 min-h-[230px] sm:min-h-[250px] md:min-h-[260px] max-h-[420px]">
              {/* Imagen y Badge */}
              <div className="relative mb-2 rounded-lg overflow-hidden h-28 sm:h-26 md:h-32 lg:h-36">
              ...
              <div className="space-y-2">
                <h3 className="font-bold text-orange-300 text-xs sm:text-sm line-clamp-2">{sale.productName}</h3>
                <p className="text-gray-400 text-[10px] sm:text-xs">{sale.category}</p>
